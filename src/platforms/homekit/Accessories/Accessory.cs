...
{
    public class Accessory : AccessoryBase, IAccessory
    {
        /// <inheritdoc />
        public IBridge ParentBridge { get; }

        protected Accessory(string uuid, string displayName, bool isBridged = false, bool isReachable = true, AccessoryCategory category = AccessoryCategory.Other) 
            : base(uuid, displayName, isBridged, isReachable, category)
        {
            Logger = Log.ForContext<Accessory>();
        }
    }
}
